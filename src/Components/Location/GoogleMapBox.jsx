...
const GoogleMapBox = ({ onSelectLocation, apiKey, latitude, longitude }) => {
  ...
  const [initialLocation, setInitialLocation] = useState({
    lat: latitude ? parseFloat(latitude) : 25.2048,
    lng: longitude ? parseFloat(longitude) : 55.2708,
  });
    ...
    const fetchData = async () => {
      try {
        ...
        if (reverseGeocodedData) {
          ...
          const { formatted_address, city, country, state } =
            reverseGeocodedData;
  ...
  const handleMapLoadError = () => {
    setMapError(
      "Failed to load the map. Please check your API key and network connection."
    );
  };
